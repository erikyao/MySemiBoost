...
def _pq_i_raw(i, y, Z, S, H):
    ...
    I_j = (y.values == 1).astype(int)
    e_H = exp(-2 * H[i])
    a = sum(reduce(np.multiply, [S_i, I_j, e_H]))

    # ----- Calculate q_i left ----- #
    # logger.debug("Calculating q_i left ...")
    I_j = (y.values == -1).astype(int)
    e_H = 1 / e_H
    a_prime = sum(reduce(np.multiply, [S_i, I_j, e_H]))
    ...
    # exp_H_diff = (H[j_u.values] - H[i]).apply(exp)
    exp_H_diff = np.exp(np.subtract(H[j_u.values], H[i]))
    b = sum(np.multiply(S_i, exp_H_diff))

    # ----- Calculate q_i right ----- #
    # logger.debug("Calculating q_i right ...")
    exp_H_diff = np.reciprocal(exp_H_diff)  # 1 / exp_H_diff
    b_prime = sum(np.multiply(S_i, exp_H_diff))
